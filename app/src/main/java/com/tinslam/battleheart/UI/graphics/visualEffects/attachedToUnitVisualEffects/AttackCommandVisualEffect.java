 ...
 */
public class AttackCommandVisualEffect extends VisualEffectAttachedToUnit{
    /**
     * Constructor.
     * @param unit The unit that the visual effect should apply to.
     */
    public AttackCommandVisualEffect(Unit unit){
    ...
    private static Animation calcAnimation(Unit unit){
        int[] images = Utils.resizeAnimation(AnimationLoader.sunStrike, unit.getCollisionBox().width());
    ...
    private static float calcY(Unit unit){
        return unit.getCollisionBox().bottom;
    }
    ...
    public float updateY(Unit unit) {
        return calcY(unit) - animation.getHeight() / 2;
    }
