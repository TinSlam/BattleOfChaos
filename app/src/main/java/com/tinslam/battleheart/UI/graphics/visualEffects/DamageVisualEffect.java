 ...
 */
public class DamageVisualEffect extends VisualEffect{
     ...
     */
    public DamageVisualEffect(float damage, float x, float y) {
        super(calcAnimation(), x, y);

        textRenderer = new TextRenderer(Utils.formatStringOneRadixPoint(Math.abs(damage)),
        ...
        if(damage < 0) textRenderer.setTextColor(Color.GREEN); else if(damage > 0) textRenderer.setTextColor(Color.RED); else textRenderer.setTextColor(Color.GRAY);
    ...
    private static Animation calcAnimation(){
        return new Animation(AnimationLoader.damageVisualEffect, 1000) {
            @Override
