...
import com.tinslam.battleheart.UI.graphics.visualEffects.SunStrike;
...
public class TrainingSpeedState extends BattleState{
    ...
    private int spawnCounter = 0, spawnInterval = 30;
    ...
    static{
        tiles = FileManager.loadFile(FileConsts.TRAINING_SPEED_FILE_NAME, 20, 20);
        if(tiles == null){
    ...
    TrainingSpeedState(String character){
    ...
    public void startBattleState() {
        ...
        switch(character){
            ...
            default:
                System.out.println("TrainingSpeedState's startBattleState() method switch command default called for the character : " + character);
        }
        new Event() {
            @Override
    ...
    public void tickBattleState() {
        ...
        if(done){
            ...
            if(spawnCounter == spawnInterval){
                spawnInterval = Utils.max(spawnInterval - 1, 5);
                spawnCounter = 0;
                    new SunStrike(spawnInterval, Utils.getRandomIntegerInTheRange(cameraRect.left, cameraRect.right, null),
                            Utils.getRandomIntegerInTheRange(cameraRect.top, cameraRect.bottom, null));
            }
        }
    }
    ...
    private void ended(){
        ...
        float msGained = Utils.calculateGainedSpeed(timer.getTime(), distanceMeasurer.getDistance());
        String character = player.getClass().getSimpleName();
        PlayerStats.setUnitSpeed(character, PlayerStats.getUnitSpeed(character) + msGained);
        PlayerStats.updateUnitStats(character);
        bonusMsRenderer = new TextRenderer(msGained + "",
                endPanelXOffset + endPanelWidth * 2 / 3,
    ...
    private void endDelayed(){
        bonusMsText = new TextRenderer(GameView.string(R.string.speed),
